...
class AsanaSync {
  ...
  /**
   * Synchronize a specific project's tasks to Firestore
   * 
   * @param {string} projectId - Project GID to sync
   * @param {string} workspaceGid - Workspace GID
   * @returns {Promise<Object>} - Sync results
   */
  async syncProject(projectId, workspaceGid) {
    if (!projectId || !workspaceGid) {
      throw new Error('Project ID and Workspace GID are required');
    }
    
    // Get project details first to include name in log
    let projectName = projectId;
    try {
      const projectData = await this.makeAsanaRequest(`/projects/${projectId}`);
      if (projectData && projectData.data) {
        projectName = projectData.data.name;
      }
    } catch (error) {
      console.warn(`Couldn't fetch project name, using ID instead: ${error.message}`);
    }
    ...
    try {
      ...
      const response = await this.makeAsanaRequest(
        `/projects/${projectId}/tasks?opt_fields=name,assignee,completed,due_on,due_at,notes,html_notes,tags,projects,parent,created_at,modified_at,completed_at,followers`
      );
      ...
      // Update the project's last sync timestamp
      await this.db.collection('asana_projects').doc(projectId).update({
        lastSyncTimestamp: firebase.firestore.FieldValue.serverTimestamp(),
        updatedAt: firebase.firestore.FieldValue.serverTimestamp()
      });
      ...
      await syncLogRef.update({
        ...
        totalTasks: stats.totalProcessed,
        newTasks: stats.created,
